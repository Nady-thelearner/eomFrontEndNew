...
import { IndexedDbService } from 'src/app/db.service';
...
export class productService {
  ...
  constructor(
    ...
    private indDBSF: IndexedDbService
  ) {}
  ...
  addCartDetails(product_Id: string) {
    ...
    this.indDBSF.addItem2({ item: this.productIds });
    this.indDBSF.addItem({ item: this.productIds });

    // this.indDBSF.updateItem({ item: this.productIds });
    var data = this.indDBSF.getAllItems();
    console.log(
      'DB data....',
      data.then((res) => console.log('response', res))
    );
    var data1 = this.indDBSF.getOneItem();
    console.log(
      'DB data....',
      data1.then((res) => console.log('responsedata1', res))
    );
